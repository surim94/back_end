...
def DOCKER_IMAGE='template/happybus-node_img'
...
podTemplate(label:label,
    ...
    node(label) {
        ...
        stage('DEPLOY') {
            container('kubectl') {
                kubeCmd.apply file: 'k8s/happybus-node-service.yaml', namespace: K8S_NAMESPACE
                yaml.update file: 'k8s/happybus-node-deployment.yaml', update: ['.spec.template.spec.containers[0].image': "${HARBOR_REGISTRY}/${DOCKER_IMAGE}:${BUILD_NUMBER}"]
                kubeCmd.apply file: 'k8s/happybus-node-deployment.yaml', wait: 300, recoverOnFail: false, namespace: K8S_NAMESPACE
            }
        }
    }
}
